...
class ElasticSink(BatchSink):
    ...
    def __init__(
        ...
        self.compiled_metadata_fields = {k: jsonpath_ng.parse(v) for k, v in (self.metadata_fields or {}).items()}
    ...
    def _template_index(self, schemas: Optional[dict] = None) -> str:
    ...
    def build_request_body_and_distinct_indices(
        ...
        """
        updated_records = []
        distinct_indices: set[str] = set()

        for record in records:
            if self.index_schema_fields:
                index = self._template_index(
                    self._build_fields(self.index_schema_fields, record, self.compiled_index_schema_fields)
                )
    ...
    def create_index(self, index: str) -> None:
        """Create an Elasticsearch index using cluster defaults or configured mappings.

        If the index already exists and ``index_mappings`` is configured, the existing
        mapping is compared and updated where possible.

        Args:
            index: Name of the index to create or update.
        """
                ...
                if not all(self.index_mappings[key]["type"] == value for key, value in mappings.items()):
                    try:
                        ...
                        if e.message == "illegal_argument_exception":
                            self.logger.warning(
                                f"Failed to update mapping for index {index}: {e}, "
                                "recreate index to apply new mappings."
                            )
                        else:
                            raise
            else:
    ...
    def process_batch(self, context: dict[str, Any]) -> None:
        ...
        Raises:
            elasticsearch.helpers.BulkIndexError: If one or more records fail to index.
                Errors are logged before being re-raised so that the loader fails
                visibly instead of silently dropping records.
        """
